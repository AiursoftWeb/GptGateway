    ...
    : IPlugin
{
    ...
    private const string AnswerPrompt =
        "你是一个旨在解决人类问题的人工智能。现在我正在调查一个问题。在调查之前，我使用搜索了 `{0}`。下面是搜索的结果。\n\n\n{1}\n\n 这些结果或许有一些参考价值吧，也可能没有。就当是补充一些你的知识了。现在，我需要你回答真正的问题：\n\n```\n{2}\n```\n\n另外，在给出答案时，如果可以，请在答案的最终附带使用 markdown 的链接格式 (也就是：[标题](URL) 的格式) 的引用部分来表达你引用的是哪条来源。";
